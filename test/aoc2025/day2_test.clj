(ns aoc2025.day2-test
  (:require [clojure.test :refer [deftest testing is are]]
            [clojure.string :as str]))

(defn- split-in-half [s]
  (let [half-length (int (Math/ceil (/ (count s) 2)))]
    (partition half-length s)))

(defn- invalid-number [coll]
  (and (= 2 (count coll)) (= (get coll 0) (get coll 1))))

(defn- id-range [s]
  (let [[start end] (map parse-long (str/split s #"-"))] (map str (range start (inc end)))))

(deftest day2

  (testing "split in half"
    (are
     [input output]
     (= output (split-in-half input))
      "11" ['(\1) '(\1)]
      "1212" ['(\1 \2) '(\1 \2)]
      "1" ['(\1)]
      "123" ['(\1 \2)]))

  (testing "invalid number"
    (are
     [input output]
     (= output (invalid-number input))
      ['(\1) '(\1)] true
      ['(\1) '(\2)] false
      ['(\1)] false))

  (testing "id range"
    (are
     [input output]
     (= output (id-range input))
      "1-2" ["1" "2"]))

  (testing "products"
    (are
     [input output]
     (= output (str/split input #","))
      "1-2,3-4" ["1-2" "3-4"]))

  (testing "e2e"
    (are
     [input output]
     (= output (->> input
                    (#(str/split % #","))
                    (map id-range)
                    (flatten)
                    (map #(into [] (split-in-half %)))
                    (filter invalid-number)
                    (map flatten)
                    (map #(parse-long (apply str %)))
                    (reduce +)))
      "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,1698522-1698528,446443-446449,38593856-38593862,565653-565659,824824821-824824827,2121212118-2121212124" 1227775554
      "52-75,71615244-71792700,89451761-89562523,594077-672686,31503-39016,733-976,1-20,400309-479672,458-635,836793365-836858811,3395595155-3395672258,290-391,5168-7482,4545413413-4545538932,65590172-65702074,25-42,221412-256187,873499-1078482,118-154,68597355-68768392,102907-146478,4251706-4487069,64895-87330,8664371543-8664413195,4091-5065,537300-565631,77-115,83892238-83982935,6631446-6694349,1112-1649,7725-9776,1453397-1493799,10240-12328,15873-20410,1925-2744,4362535948-4362554186,3078725-3256936,710512-853550,279817-346202,45515-60928,3240-3952" 26255179562)))
