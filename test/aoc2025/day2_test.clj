...
(defn partition-string [n s]
  (map #(apply str %) (partition n n (repeat " ") s)))

(defn- invalid-number [s]
  (let [parts (range 1 (int (+ 1 (Math/ceil (/ (count s) 2)))))]
    (->> parts
         (map #(partition-string % s))
         (map #(and (> (count %) 1) (apply = %)))
         (reduce #(or %1 %2)))))
